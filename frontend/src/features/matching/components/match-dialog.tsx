...
// pick. Keyed by `${requirement_id}:${original holder's user_id}`, NOT by
// requirement_id alone: a requirement with required_count > 1 can be covered by two
// different team members, each with their own seats[] entry carrying the same
// requirement_id, so requirement_id alone can't tell those two seats apart. Absent
// entries mean "still whoever the matcher proposed."
type Swap = { user_id: number; name: string };

function swapKey(requirementId: number, holderUserId: number) {
  return `${requirementId}:${holderUserId}`;
}
...
export function MatchDialog({ missionId, open, onOpenChange }: MatchDialogProps) {
  ...
  function handleSwap(requirementId: number, holderUserId: number, candidateUserId: string | null) {
    ...
    setSelected((prev) => {
      const next = new Set(prev);
      next.delete(holderUserId);
      next.add(candidate.user_id);
      return next;
    });
    setSwaps((prev) => ({
      ...prev,
      [swapKey(requirementId, holderUserId)]: { user_id: candidate.user_id, name: candidate.name },
    }));
  }
            ...
            {result.team.length === 0 ? (
              ...
              <ul className="flex flex-col gap-3">
                {result.team.map((member) => (
                  <li key={member.user_id} className="flex flex-col gap-2 rounded-lg border p-3">
                          ...
                          <div key={seat.requirement_id} className="flex flex-wrap items-center gap-2">
                            ...
                            {alt && alt.candidates.length > 0 && (
                              <Select value={swap ? String(swap.user_id) : ""} onValueChange={(value) =>
                                handleSwap(seat.requirement_id, member.user_id, value)
                              }>
                                <SelectTrigger
                                  size="sm"
